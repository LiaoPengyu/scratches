...
def main():
    ...
    for i in range(1,2):
        ...
        for j in range(0,1):
            ...
            n = n1 * (j+1)
            msn=int(n*ms)
            args="dumbo start ./cand.py -hadoop "+hpath+" -input "+dpath+" -output /user/hadoop/tina/fis -param ms="+str(ms)+" msn="+str(msn)+" p="+str(p)+" -overwrite yes"
            args1 ="dumbo cat /user/hadoop/tina/fis -hadoop "+hpath+" > cand0.txt"
            os.system(args)
            os.system(args1)
            idenCan(ms,msn,n)
            args3="dumbo start ./fis.py -hadoop "+hpath+" -input "+dpath+" -output /user/hadoop/tina/afis -overwrite yes  -param ms="+str(ms)+" msn="+str(msn)+" -file 'cand1.txt' -file 'fc1.txt'"
            args4 ="dumbo cat /user/hadoop/tina/afis -hadoop "+hpath+" > fis.txt"
            os.system(args3)
            os.system(args4)
