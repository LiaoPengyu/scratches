...
class FPNode(object):
    ...
    def merge(self, other):
        ...
        for child in other.children:
            if child.item in self._children.keys():
        ...
        for docID,doc in data:
            ...
            if n==self.p:
                ...
                for itemsets in find_frequent_itemsets(trans, msn):
                    itemsets[0].sort()
                    yield itemsets[0],(itemsets[1],n, msn)
                trans=[]
                n=0
        if n>0:
            msn = int(n*float(self.min_sup)*0.9)
...
class Reducer():
    ...
    def __call__(self,key,values):
        ...
        for v1, v2, msn in values:
            yield key, ":" + ' '.join((str(v1), str(v2), str(msn), str(c)))
            c += 1

        #sum1, sum2 = reduce(lambda v1,v2:(v1[0]+v2[0], v1[1]+v2[1]), values)
        #yield key, ''.join((':', str(sum1), ' ', str(sum2)))
